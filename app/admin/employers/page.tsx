              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>{employers.map((em) => (
                    <tr key={em.id} className="border-b hover:bg-slate-50 transition-colors"><td className="p-4 font-medium">{em.company_name || '-'}</td><td className="p-4">{em.contact_person || '-'}</td><td className="p-4">{em.email}</td><td className="p-4"><span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${em.validation_status === 'VALIDATED' ? 'bg-green-100 text-green-800' : em.validation_status === 'PENDING' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>{em.validation_status || 'N/A'}</span></td><td className="p-4 text-right"><Button variant="outline" size="sm" onClick={() => openDetailModal(em)} className="flex items-center gap-1"><Eye className="h-4 w-4" />Détails</Button></td></tr>
                  ))}</tbody>
                </table>
              </div>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full p-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-8">
              ...
              <div className="flex gap-3 mt-8"><Button variant="outline" onClick={() => { setShowDetailModal(false); setSelectedEmployer(null); setSelectedProfile(null); }} className="flex-1">Fermer</Button><Button variant={selectedEmployer.blocked ? "default" : "destructive"} onClick={toggleBlockStatus} disabled={editLoading} className="flex-1">{editLoading ? 'Traitement...' : selectedEmployer.blocked ? 'Débloquer' : 'Bloquer'}</Button><Button onClick={openEditModal} disabled={!selectedProfile} className="flex-1">Modifier</Button></div>
            </div>
          </div>
        </div>
      )}
